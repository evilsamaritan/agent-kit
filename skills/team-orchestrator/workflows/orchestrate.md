...
### 1a. Load saved teams
```
Glob .claude/teams/*/team.json (if any)
...
Glob agents/*.md → for each file:
  ...
  - Classify: base profession agent vs custom specialist agent
  - Base = agents/<profession>.md (architect, frontend, backend, devops, sre,
    security, tester, designer, reviewer, writer)
  - Custom = any other agent with its own skills[] composition
```
...
User wants multiple copies of the same agent: "run 3 tester agents on different modules":
...
Agent tool calls (parallel):
  subagent_type: "tester", prompt: "Review tests in api/", run_in_background: true
  subagent_type: "tester", prompt: "Review tests in web/", run_in_background: true
  subagent_type: "tester", prompt: "Review tests in workers/", run_in_background: true
```
...
1. Search `.claude/teams/<name>/team.json` for exact name match
...
4. Present proposed team to user:
   ...
   4. tester — write tests (sequential)

   Agents: architect, backend, frontend, security, tester
   Flow: pipeline (1 → 2∥ → 3 → 4)

   Confirm? Modify? Save for reuse?
   ```

5. If user confirms and wants to save → delegate to `team-creator` (writes `.claude/teams/<name>/team.json`)
...
User listed specific agents: "use architect, tester, and security":
...
```
architect (plan) → builder-1 (implement) → validator-1 (review) → builder-1 (fix) → tester (test)
```
...
## Step 6: Persist (Optional) — delegate to team-creator

Team definitions live under `.claude/teams/<name>/team.json` and are owned by `team-creator`. This skill never writes them.

If an ad-hoc composition (Path B or C) proved useful:

1. Ask the user: "Save this team composition for future use?"
2. If yes, hand off to `team-creator` via the Skill tool with the resolved composition as input:
   - `agents` list
   - `flow` type (`pipeline`, `pipeline-parallel`, `twin-review`, ...)
   - ordered `stages` (agent + mode + tools + scope)
3. `team-creator` writes `.claude/teams/<name>/team.json`, runs `validate-team.sh`, and reports back the final path.
4. Confirm to the user: "Team '{name}' saved at .claude/teams/{name}/team.json. Re-run with `/team-orchestrator {name}` next time."
